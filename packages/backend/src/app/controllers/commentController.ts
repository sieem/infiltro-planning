...
export const saveComment = async (req: Request, res: Response) => {
    ...
    if (!commentForm._id) {
        commentObject = {
            _id: new Types.ObjectId() as unknown as ObjectId,
        ...
        try {
            await Project.updateOne({ _id: req.params.projectId }, { $push: { comments: commentObject as never } }).exec()
        } catch (error: any) {
            console.log(error)
        }
...
export const removeComment = (req: Request, res: Response) => {
    Project.findById(req.params.projectId, async (err: any, project: IProject) => {
        if (err) {
            console.error(err)
            return res.status(400).json(err.message)
        }
        if (req.user?.role !== 'admin' && getComment(project.comments, req.params.commentId as unknown as ObjectId)?.user !== req.user?._id) {
            return res.status(401).send('Unauthorized request');
        }
        project.comments = removeElementInArray(project.comments, req.params.commentId as unknown as ObjectId)
        await Project.findByIdAndUpdate(req.params.projectId, project, { upsert: true }).exec()
        return res.status(200).json(project.comments)
    })
}

function removeElementInArray(array: IComment[], commentId: ObjectId): IComment[] {
...
function getComment(array: IComment[], commentId: ObjectId): IComment | undefined {
